    ...
    class JSOL {
        public static function dict(...$args) {
            ...
            for ($i = 0; $i < count($args); $i += 2) {
                if (array_key_exists($i + 1, $args)) { $obj[$args[$i]] = $args[$i + 1]; }
            }
            return $obj;
        }
...
function compileJsolInMemory(string $sourcePath, string $compilerDir, string $outDir, array $metadata): array {
    ...
    $parts = [
        'lexer.php', 'linter.php', 'cli-parser.php', 
        'config-parser.php', 'regex.php', 'js-compiler.php', 
        'php-compiler.php', 'engine.php'
    ];
