...
const $saInd_RepeatUnit = function($saUnit, $iCount) {
  let $saOut = "";
...
const $bInd_IsWhitespace = function($saCh) {
  return $saCh === " " || $saCh === "\t" || $saCh === "\n" || $saCh === "\r";
};
const $saInd_RTrimBuffer = function($saBuf) {
  let $iEnd = Str["len"]($saBuf);
    while ($iEnd > 0 && $bInd_IsWhitespace(Str["sub"]($saBuf,  $iEnd - 1,  1)) === true) {
    $iEnd = $iEnd - 1;
  }
  return Str["sub"]($saBuf,  0,  $iEnd);
};
const $iInd_SkipWhitespace = function($saCode, $iStart, $iLen) {
  let $i = $iStart;
...
const $saIndentCode = function($saMaskedCode, $saIndentUnit) {
  let $saResult = "";
    let $iDepth = 0;
    let $i = 0;
    ...
    while ($i < $iLen) {
    const $saChar = Str["sub"]($saMaskedCode,  $i,  1);
