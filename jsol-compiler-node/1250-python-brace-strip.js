...
const $bIsWhitespaceCharPY = function($saCh) {
  if ($saCh === " ") {
    return true;
  }
  if ($saCh === "\t") {
...
const $saTrimWithPY = function($saVal) {
...
const $saStripPythonBraces = function($saIndentedCode, $saIndentUnit) {
  let $saResult = "";
    let $iDepth = 0;
    let $i = 0;
    const $iLen = Str["len"]($saIndentedCode);
    let $bStartOfLine = true;
    let $aOpenPositions = [];

    while ($i < $iLen) {
    const $saCh = Str["sub"]($saIndentedCode,  $i,  1);
      ...
      const $saSinceOpen = Str["sub"]($saResult,  $iContentStart,  Str["len"]($saResult) - $iContentStart);
