...
const $bEndsWith = function ($sStr, $sSuffix) {
  const $iStrLen = $sStr.length;
	const $iSufLen = $sSuffix.length;
	if ($iSufLen > $iStrLen) {
    return false;
  }
...
const $mParseRawCliArgs = function ($aRawArgs) {
  const $mOptions = JSOL.dict(
    ...
    const $sArg = $aRawArgs[$i];
		const $bIsFlag = ($sArg.indexOf( "--") === 0);

		if ($bIsFlag === true) {
      const $sClean = $sArg.substring( 2, ( 2) + ( $sArg.length - 2));
      ...
      if ($sKey === "source") {
        ...
        const $sRawId = $sKey.substring( 0, ( 0) + ( $sKey.length - 7));
