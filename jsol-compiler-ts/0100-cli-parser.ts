...
const $bCli_EndsWith = function($saStr: any, $saSuffix: any): boolean {
  const $iStrLen: number = Str["len"]($saStr);
	const $iSufLen: number = Str["len"]($saSuffix);
	if ($iSufLen > $iStrLen) {
    return false;
  }
  return Str["sub"]($saStr,  $iStrLen - $iSufLen,  $iSufLen) === $saSuffix;
};
const $saCli_NormalizeTargetId = function($saId: any): string {
  if ($saId === "py") {
    return "python";
  }
  return $saId;
...
const $mParseRawCliArgs = function($aRawArgs: any): Record<string, any> {
  const $mOptions: Record<string, any> = JSOL.dict("source",  "",  "sourceDir",  "",  "outDir",  "",  "target",  "",  "targets",  "");
	const $iCount: number = $aRawArgs.length;

	for (let $i = 0; $i < $iCount; $i = $i + 1) {
    const $saArg: string = $aRawArgs[$i];
		if (Str["indexOf"]($saArg,  "--") === 0) {
      const $saClean: string = Str["sub"]($saArg,  2,  Str["len"]($saArg) - 2);
			const $iEqIndex: number = Str["indexOf"]($saClean,  "=");
			let $saKey: string = $saClean;
			let $saVal: string = "true";

			if ($iEqIndex !== -1) {
        $saKey = Str["sub"]($saClean,  0,  $iEqIndex);
				$saVal = Str["sub"]($saClean,  $iEqIndex + 1,  Str["len"]($saClean) - ($iEqIndex + 1));
      }
      if ($saKey === "source") {
        ...
        $mOptions[Str["concat"]($saCli_NormalizeTargetId(Str["sub"]($saKey,  0,  Str["len"]($saKey) - 7)),  "Target")] = $saVal;
      }
      else if ($bCli_EndsWith($saKey, "-prefix") === true) {
        $mOptions[Str["concat"]($saCli_NormalizeTargetId(Str["sub"]($saKey,  0,  Str["len"]($saKey) - 7)),  "Prefix")] = $saVal;
      }
      else if ($bCli_EndsWith($saKey, "-suffix") === true) {
        $mOptions[Str["concat"]($saCli_NormalizeTargetId(Str["sub"]($saKey,  0,  Str["len"]($saKey) - 7)),  "Suffix")] = $saVal;
      }
    }
  }
  return $mOptions;
