...
const $mParseAtom = function($sPat: any, $i: any, $iN: any, $iGc: any, $mFns: any): Record<string, any> {
  ...
  if ($sC === "\\") {
    $i = $i + 1;
        ...
        $i = $i + 1;
        if ($sE === "d") {
      const $aRd: any[] = [["0", "9"]]; return JSOL.dict("node",  JSOL.dict("type",  "class",  "negate",  false,  "ranges",  $aRd,  "singles",  []),  "i",  $i,  "groupCount",  $iGc);
    }
    if ($sE === "w") {
      const $aRw: any[] = [["a", "z"], ["A", "Z"], ["0", "9"]]; const $aSw: any[] = ["_"]; return JSOL.dict("node",  JSOL.dict("type",  "class",  "negate",  false,  "ranges",  $aRw,  "singles",  $aSw),  "i",  $i,  "groupCount",  $iGc);
    }
    if ($sE === "s") {
