...
const $mExecuteCompilationPipeline = function($sSourceCode: any, $mTargetsConfig: any, $mCliOptions: any, $mSSOT: any): Record<string, any> {
    ...
    const $sFinalJS: string = $sUnmaskSourceCode($sCompiledJS, $aTokens);
    const $sFinalPHP: string = $sUnmaskSourceCode($sCompiledPHP, $aTokens);
    const $sFinalTS: string = $sUnmaskSourceCode($sCompiledTS, $aTokens);
