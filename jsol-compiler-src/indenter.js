...
const $sRepeatUnit = function($sUnit, $iCount) {
  let $sOut = "";
...
const $bIsWhitespaceChar = function($sCh) {
  if ($sCh === " ") {
    return true;
  }
  if ($sCh === "\t") {
...
const $sRTrimBuffer = function($sBuf) {
  let $iEnd = $sBuf.length;
    while ($iEnd > 0 && $bIsWhitespaceChar($sBuf.substring( $iEnd - 1, ( $iEnd - 1) + ( 1))) === true) {
    $iEnd = $iEnd - 1;
  }
  return $sBuf.substring( 0, ( 0) + ( $iEnd));
};
const $sIndentCode = function($sMaskedCode, $sIndentUnit) {
  let $sResult = "";
    let $iDepth = 0;
    let $i = 0;
    ...
    while ($i < $iLen) {
    const $sChar = $sMaskedCode.substring( $i, ( $i) + ( 1));
