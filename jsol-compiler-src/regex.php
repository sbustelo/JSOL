...
$mParseAtom = function($sPat, $i, $iN, $iGc, $mFns) {
  $sC = mb_substr($sPat,  $i,  1, "UTF-8");
    ...
    $i = $i + 1;
        $bCapturing = true;
        if ($i + 1 < $iN && mb_substr($sPat,  $i,  2, "UTF-8") === "?:") {
      $bCapturing = false;
            $i = $i + 2;
    }
    $iIdx = -1;
  ...
  if ($sC === "\\") {
    $i = $i + 1;
        ...
        $i = $i + 1;
        if ($sE === "d") {
      $aRd = [["0", "9"]]; return JSOL::dict("node",  JSOL::dict("type",  "class",  "negate",  false,  "ranges",  $aRd,  "singles",  []),  "i",  $i,  "groupCount",  $iGc);
    }
    if ($sE === "w") {
      $aRw = [["a", "z"], ["A", "Z"], ["0", "9"]]; $aSw = ["_"]; return JSOL::dict("node",  JSOL::dict("type",  "class",  "negate",  false,  "ranges",  $aRw,  "singles",  $aSw),  "i",  $i,  "groupCount",  $iGc);
    }
    if ($sE === "s") {
...
$fGen = function($mN, $aProg, $fSelfFn) {
  $sType = $mN["type"];
    ...
    if (isset($mN[ "capturing"]) && $mN["capturing"] === false) {
      $aProg = $fSelfFn($mN["body"], $aProg, $fSelfFn);
    }
    else {
...
$mRegexMatch = function($sPatternStr, $sStr, $sFlags) use (&$mParsePattern, &$aCompileRegex, &$mRunRegex) {
  $bCi = false;
    $bGlobal = false;
    if (JSOL::strIndexOf($sFlags,  "i") !== -1) {
    $bCi = true;
  }
  if (JSOL::strIndexOf($sFlags,  "g") !== -1) {
    $bGlobal = true;
  }
...
$sRegexReplace = function($sPatternStr, $sReplacementStr, $sStr, $sFlags) use (&$mParsePattern, &$aCompileRegex, &$mRunRegex) {
  ...
  $mParsed = $mParsePattern($sPatternStr);
    ...
    while ($i <= $iN) {
    $bMatchFound = false;
        $mR = JSOL::dict("matched",  false,  "saves",  []);
        $iMatchIndex = $i;
...
$bRegexTest = function($sPatternStr, $sStr, $sFlags) {
  $mR = $mRegexMatch($sPatternStr, $sStr, $sFlags);
    if (isset($mR[ "matched"]) && $mR["matched"] === true) {
    return true;
  }
  return false;
};
