...
> Given identical input payloads, the transpilations of a JSOL file MUST yield **bit-for-bit identical output** in supported targets (as JSOL v0.2.95: JavaScript (V8), PHP 8.x (Zend Engine), TypeScript and Python 3).

Before writing or approving any line of code, evaluate this question: **"Will this code produce divergent results in PHP versus JavaScript, TypeScript and/or Python due to fundamental engine physics or regex transpilation limits?"** If YES or UNCERTAIN, discard the pattern and apply a zero-divergence JSOL structure instead.

### Prevent Fundamental Engine Desynchronizations:

When analyzing or refactoring code for JSOL, simulate execution in **V8 (JavaScript)**, **Zend Engine (PHP 8.x)** and **Python 3** to catch these before they ship:
...
6. **String Concatenation:** Never use bare '+' for string concatenation. Use template literals, the '+ "" +' pattern, or Str.concat( $a, $b, ...). Str.concat() allows to concatenate strings and numbers without a Fatal error in Python.
